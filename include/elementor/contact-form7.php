 ...
 * @since 1.0.0
 */
class Od_Contactform7 extends Widget_Base
{
    ...
    {
        return __('Contact Form7', 'ordainit-toolkit');
    }
    ...
    public function get_od_contact_form()
    {
        ...
        $od_cfa         = ['0' => esc_html__('Select Form', OD)];
        if ($od_forms) {
            foreach ($od_forms as $od_form) {
                $od_cfa[$od_form->ID] = $od_form->post_title;
            }
        } else {
            $od_cfa[esc_html__('No contact form found', OD)] = 0;
        }
        return $od_cfa;
    }
    ...
    {
        include_once(ORDAINIT_TOOLKIT_ELEMENTS_PATH . '/control/contact-form7.php');
    }
...
$widgets_manager->register(new Od_Contactform7());
